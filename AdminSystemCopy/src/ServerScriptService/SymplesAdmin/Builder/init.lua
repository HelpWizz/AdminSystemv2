...
return function(Settings)
	...
	remotes.Function.Parent, remotes.Event.Parent, remotes.BindableEvent.Parent =
	...
	local function buildTempPermissions(permissions, group, groupconfig)
		...
		then
			...
			local inherited =
				buildTempPermissions(permissions, groupconfig["Inherits"], permissions[groupconfig["Inherits"]])
	...
	local function loadPackages()
		...
		local allowedFolderNames = { "Character", "Messaging", "Moderation", "logs", "other" }
		for _, v in pairs(script.Packages:GetDescendants()) do
			if
				v:IsA("ModuleScript")
				and v.Parent.ClassName ~= "ModuleScript"
				and table.find(allowedFolderNames, v.Parent.Name)
			then
	...
	if not script.Library.UI.Themes:FindFirstChild(systemPackages.Settings["Settings"].Misc.Theme) then
		warn("ERROR! | Theme " .. systemPackages.Settings["Settings"].Misc.Theme .. " is not installed")
		warn("Switching to default theme...")
		assert(script.Library.UI.Themes:FindFirstChild("aphotic"), "Default theme missing...")
		local themeColorValue = Instance.new("Color3Value")
		themeColorValue.Name = "ThemeColor"
		themeColorValue.Value = systemPackages.Settings["Settings"].Misc.DefaultThemeColor

		currentTheme = script.Library.UI.Themes:FindFirstChild("aphotic"):Clone()
		currentTheme.Name = "StyleSheet"
		themeColorValue.Parent = currentTheme
		currentTheme.Parent = script.Library.UI.Client.Scripts.Library.Modules
	else
		local themeColorValue = Instance.new("Color3Value")
		themeColorValue.Name = "ThemeColor"
		themeColorValue.Value = systemPackages.Settings["Settings"].Misc.DefaultThemeColor

		currentTheme = script.Library.UI.Themes:FindFirstChild(systemPackages.Settings["Settings"].Misc.Theme):Clone()
		currentTheme.Name = "Stylesheet"
		themeColorValue.Parent = currentTheme
		currentTheme.Parent = script.Library.UI.Client.Scripts.Library.Modules
	end
	...
	local function loadUiTheme(Client)
		for _, v in pairs(Client.PlayerGui.Client.Scripts.Library.Components:GetChildren()) do
			if v:IsA("ModuleScript") then
				local mod = require(v)
				mod.Init()
			end
		...
		for _, i in pairs(Client.PlayerGui.Client.Client:GetDescendants()) do
			if i:isA("ModuleScript") and i.Name == "handler" then
				local mod = require(i)
				mod.Stylesheet = require(script.Library.UI.Client.Scripts.Library.Modules.Stylesheet)
				mod.Theme()
			end
		end
	end
	...
	local function lastCalledFunction(player: Player)
		...
		if getPlayerAdminRank ~= "None" then
			Api.Players.notify(
				player,
				"System",
				string.format(
					"Your current Admin Level is %q \n%q for the list if commands",
					getPlayerAdminRank,
					":cmds"
				)
			)
