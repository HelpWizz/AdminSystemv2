...
module.Execute = function(Client, Type, Attachment, value)
	if Type == "command" then
		...
		if module.Settings["Settings"].SpecialAdminSettings[playerRank] then
			if module.Settings["Settings"].SpecialAdminSettings[playerRank]["RemoveHats"] then
				commandDissallowedAliases = module.Settings["Settings"].SpecialAdminSettings[playerRank]["RemoveHats"]
			end
		end
		...
		if players ~= nil then
			for i, v in pairs(players) do
				...
				if backpack then
					for _, v in ipairs({v.Character and table.unpack(v.Character:GetChildren()), table.unpack(backpack:GetChildren())}) do
			...
			module.SetWaypoint.new(Client, "remove tools", Attachment)
			return true
		else
